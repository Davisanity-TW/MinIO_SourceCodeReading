...
以你目前 workspace 的 MinIO source tree（`/home/ubuntu/clawd/minio`）為準，`addPartial()` 最典型的觸發點是在 `erasureObjects.putObject()` **完成 tmp rename + commit 之後**：
...
（你可以在 `/home/ubuntu/clawd/minio/cmd/erasure-object.go` 看到這段邏輯緊接在 `commitRenameDataDir` 之後。）
